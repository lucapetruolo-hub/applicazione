# Professionisti — Changelog dettagliato

> **Cosa è questo file**: lo storico completo, sezione per sezione, di ogni
> modifica fatta al prodotto dalla Fase 0 del redesign in poi (§10 in avanti —
> §1-9, le regole architetturali vive, restano in `CLAUDE.md`). Ogni voce
> descrive la richiesta, la decisione presa, il perché, e com'è stata
> verificata — pensato per essere consultato **su richiesta**, non caricato
> automaticamente ad ogni turno di conversazione.
>
> **Quando consultarlo**: prima di modificare un'area toccata da un bug già
> risolto in passato (per non reintrodurlo), per capire perché un componente
> è fatto in un certo modo, o per ricostruire il contesto di una decisione di
> prodotto specifica. Non serve leggerlo per intero — usa Grep sul numero di
> sezione o sulle parole chiave dell'area che ti interessa.
>
> **Regola per chi aggiorna questo file**: le nuove voci vanno qui, non in
> `CLAUDE.md` — `CLAUDE.md` resta lo stato attuale/le regole vive (stack,
> architettura, regole di sviluppo, elenco funzionalità con stato), mai un
> diario. Se una voce introduce o cambia una **regola vincolante permanente**
> (es. un principio di privacy, un vincolo architetturale da non invertire
> senza discuterne), quella regola va comunque riportata in forma sintetica
> anche in `CLAUDE.md` §5 — questo file resta la sola narrazione/motivazione
> estesa.
